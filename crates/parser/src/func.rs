...
fn is_self(syntax: Syntax) -> bool {
    syntax.is_of_kind(Kwself)
}
...
impl<'input> Parser<'input> {
    ...
    pub fn parse_function_def(&self) -> Option<Finished> {
        ...
        self.parse_block();
        println!(" parsed block : {:?}", self.peek());
    ...
    pub fn parse_comma_separated_typed_declarations_until(&self, until_false: fn(Syntax) -> bool) {
        ...
        let arg_elements = thin_vec![Kind(Ident), Kind(Colon), Fn(ident_or_type)];
        let can_be_a_method = thin_vec![Branched(thin_vec![Fn(is_self)], arg_elements)];
        self.parse_arg(&can_be_a_method);
        println!("parsed first arg {:?}", self.peek());

        let arg_elements = thin_vec![Kind(Ident), Kind(Colon), Fn(ident_or_type)];
        let can_be_a_method = thin_vec![Branched(
            thin_vec![Fn(is_self)],
            thin_vec![Kind(Ident), Kind(Colon), Fn(ident_or_type)],
        )];
        self.parse_arg(&can_be_a_method);
    ...
    fn parse_ref_arg(&self, elements: &ThinVec<SeparatedElement>) {
        let marker = self.start();
    ...
    fn parse_mut_arg(&self, elements: &ThinVec<SeparatedElement>) {
    ...
    fn parse_arg_with(&self, elements: &ThinVec<SeparatedElement>) {
        // if we give Comma, it will be ignored within
        let dummy_separator = Root;
        self.parse_separated_by(elements, ParamDecl, dummy_separator, |syntax| {
            !syntax.is_of_kind(Comma) && !syntax.is_of_kind(RParen)
        });
